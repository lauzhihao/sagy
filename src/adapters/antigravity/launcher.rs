...
pub enum LaunchError {
    ...
    /// Mirroring a stderr chunk to the parent failed.  The drain still runs to
    /// EOF before this error is returned, preventing a child pipe deadlock.
    DrainWrite(io::ErrorKind),
...
impl fmt::Display for LaunchError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::DrainWrite(kind) => write!(
                formatter,
                "failed to mirror Antigravity CLI stderr ({kind:?})"
            ),
...
fn configure_auth_environment(command: &mut Command, auth: &LaunchAuth, project_id: Option<&str>) {
    ...
    command.env_remove("GOOGLE_CLOUD_PROJECT");
    match auth {
        LaunchAuth::OAuth => {
    ...
    W: Write + Send + 'static,
{
    ...
    if let Some(kind) = report.writer_error {
        return Err(LaunchError::DrainWrite(kind));
    }
...
mod tests {
    ...
    fn auth_environment_is_an_exact_four_kind_matrix() {
        ...
        for (auth, project, expected_key, expected_credentials, expected_project) in cases {
            ...
            configure_auth_environment(&mut command, &auth, project);
            let value = |name: &str| {
                command
                    .get_envs()
    ...
    mod observed {
        ...
        fn fragmented_json_from_fake_child_is_classified_after_wait() {
            ...
            assert_eq!(run.buffered_len, RATE_LIMIT_JSON.len());
        }
